...
function Toast({ message, onDone }) {
  ...
  return <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-full bg-gray-900 text-white text-sm shadow-lg">{message}</div>;
}
...
function RealSparkline({ data, isPositive }) {
  ...
  return (
    <svg width="58" height="26" viewBox="0 0 100 100" className="shrink-0">
      <polyline fill="none" stroke={isPositive ? "#10b981" : "#ef4444"} strokeWidth="7" strokeLinecap="round" strokeLinejoin="round" points={points} />
    </svg>
  );
}
...
function AnimatedPrice({ value }) {
  ...
  return <span className={`text-lg font-semibold tabular-nums transition-colors ${flash === "up" ? "text-emerald-600" : flash === "down" ? "text-red-600" : "text-gray-900"}`}>{value !== "—" ? "$" + value : "—"}</span>;
}
  ...
  return (
    <Link to={`/stock/${item.ticker}`} className="block rounded-2xl bg-white border border-gray-200 shadow-sm px-4 py-4 active:scale-[0.99] transition">
      <div className="flex items-center justify-between gap-3">
        <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onStarToggle(item, stock); }} className="p-1 min-h-[44px] min-w-[36px] flex items-center justify-center shrink-0">
        ...
        <div className="min-w-0 flex-1">
          ...
          {item.exchange && <span className="text-[11px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">{abbreviateExchange(item.exchange)}</span>}
        </div>
...
export default function Watchlist() {
  ...
  const loadSparklines = async (tickers) => {
    ...
    await Promise.all(tickers.map(async (t) => {
      try {
        const res = await callFinnhub({ action: "candles_range", ticker: t, resolution: "D", from: Math.floor(Date.now() / 1000) - 32 * 86400, to: Math.floor(Date.now() / 1000) });
  ...
  const handleAdd = async (newTicker) => {
    ...
    if (items.some(i => i.ticker.toUpperCase() === upperTicker)) { setToast("Already in your watchlist"); setTicker(""); return; }
    setAdding(true);
    try {
      let companyName = upperTicker, exchange = "";
      try {
      ...
      const { data, error } = await supabase.from("watchlist_items").insert({ user_id: user.id, ticker: upperTicker, company_name: companyName, exchange }).select().single();
      ...
      setTicker("");
      setShowSuggestions(false);
    } catch { setToast("Failed to add ticker"); }
    finally { setAdding(false); }
  };

  const handleStarToggle = (item, stock) => {
    setDialogItem({ ticker: item.ticker, companyName: getCompanyName(item.ticker, stock, item) });
  };

  const findStock = (tickerValue) => stocks.find(s => s.ticker.toUpperCase() === tickerValue.toUpperCase());
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      {dialogItem && (
        <AddToPortfolioDialog open={!!dialogItem} onOpenChange={() => setDialogItem(null)} ticker={dialogItem.ticker} companyName={dialogItem.companyName} onAdded={async () => { const { data } = await supabase.from("stocks").select("*").eq("user_id", user?.id); if (data) setStocks(data); setDialogItem(null); }} userId={user?.id} />
      )}

      <div className="max-w-2xl mx-auto px-4 pt-6">
        <div className="mb-6 flex flex-col items-center text-center">
          <div className="flex items-center gap-2"><Star className="h-7 w-7 text-yellow-500 fill-yellow-500" /><h1 className="text-3xl font-bold">Watchlist</h1></div>
        ...
        <form onSubmit={e => { e.preventDefault(); if (ticker.trim()) handleAdd(ticker); }} className="mb-6">
          <div className="relative">
            <div className="flex gap-2">
              <Input ref={inputRef} value={ticker} onChange={handleInputChange} onFocus={() => setShowSuggestions(true)} onBlur={() => setTimeout(() => setShowSuggestions(false), 150)} placeholder="Add ticker (e.g. AAPL)" className="flex-1" disabled={adding} />
              <Button type="submit" disabled={adding || !ticker.trim()}>{adding ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Add</Button>
            </div>
            <AnimatePresence>
              {showSuggestions && (suggestions.length > 0 || searchLoading) && (
                <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute z-[60] mt-1.5 w-full rounded-2xl border bg-white shadow-xl overflow-hidden">
                  {searchLoading && <div className="px-4 py-3 text-sm text-gray-500">Searching...</div>}
                  {suggestions.map((s, i) => (
                    <button key={i} type="button" onClick={() => handleSelectSuggestion(s)} className="w-full px-4 py-3 text-left hover:bg-gray-50 flex justify-between text-sm">
                      <div><span className="font-semibold">{s.symbol}</span> <span className="text-gray-500">{s.description}</span></div>
        ...
        {loading ? <div className="flex justify-center py-16"><Loader2 className="animate-spin" /></div> : items.length === 0 ? (
          <div className="rounded-2xl border border-dashed p-12 text-center">Nothing here yet. Add a ticker above.</div>
        ) : (
          <div className="space-y-3">
            {items.map(item => (
              <WatchlistCard key={item.id} item={item} stock={findStock(item.ticker)} quote={quotes[item.ticker?.toUpperCase()]} sparklineData={sparklines[item.ticker?.toUpperCase()]} onStarToggle={handleStarToggle} />
            ))}
          </div>
        )}
      </div>
