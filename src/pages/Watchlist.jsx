...
import { motion, AnimatePresence } from "framer-motion";import React, { useState, useEffect, useRef } from "react";
import { supabase } from "@/lib/supabase";
import { useAuth } from "@/lib/AuthContext";
import { useMarketData } from "@/lib/MarketDataContext";
...
export default function Watchlist() {
    ...
    await Promise.all(
      tickers.map(async (t) => {
        try {
          ...
          console.log("Sparkline response for", t, res); // ← Temporary debug (remove later)

          if (res?.candles && Array.isArray(res.candles) && res.candles.length > 1) {
            // ✅ Using c.v because your backend puts close price in "v"
            newData[t] = res.candles.map((c) => ({ close: c.v }));
          }
        } catch (err) {
          console.error("Sparkline fetch failed for", t, err);
        }
      })
    );
  ...
  const load = async () => {
    ...
    if (watchData.length > 0) {
      ...
      fetchQuotes(tickers);           // Fixes price population
