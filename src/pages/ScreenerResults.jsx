  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl px-4 py-3 space-y-3">
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-muted-foreground/60 uppercase tracking-wide">{stock.exchange}</span>
          </div>
          <p className="text-xs text-muted-foreground">{stock.name}</p>
          {stock.sector && <p className="text-[10px] text-muted-foreground/50">{stock.sector}</p>}
        </div>
        <div className="text-right shrink-0">
          <p className="font-semibold text-sm">${stock.price?.toFixed(2) ?? "—"}</p>
          {stock.changePercent != null && (
            <div className={`inline-flex items-center gap-0.5 text-xs font-semibold ${changePos ? "text-emerald-600" : "text-red-600"}`}>
              {changePos ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
              {changePos ? "+" : ""}{stock.changePercent.toFixed(2)}%
            </div>
          )}
        </div>
        <button
      ...
      <div className="grid grid-cols-4 gap-2 border-t border-gray-50 pt-2 sm:grid-cols-8">
        <Metric label="52W Chg" value={stock.week52Change != null ? <span className={week52Pos ? "text-emerald-600" : "text-red-600"}>{week52Pos ? "+" : ""}{stock.week52Change.toFixed(1)}%</span> : null} />
        <Metric label="P/E" value={stock.pe != null ? stock.pe.toFixed(1) : null} />
        <Metric label="EPS" value={stock.eps != null ? `$${stock.eps.toFixed(2)}` : null} />
        <Metric label="D/E" value={stock.deRatio != null ? stock.deRatio.toFixed(2) : null} />
        <Metric label="Mkt Cap" value={stock.marketCapB != null ? `$${stock.marketCapB.toFixed(1)}B` : null} />
        <Metric label="Div Yield" value={stock.dividendYield != null ? `${stock.dividendYield.toFixed(2)}%` : null} />
...
export default function ScreenerResults() {
  ...
  const addToWatchlist = async (stock) => {
    try {
      const existing = await base44.entities.WatchlistItem.filter({ ticker: stock.ticker, created_by_id: user?.id });
      if (existing.length > 0) { showToast(`${stock.ticker} already in watchlist`); return; }
      await base44.entities.WatchlistItem.create({ ticker: stock.ticker, company_name: stock.name, exchange: stock.exchange });
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 flex flex-col" style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + 64px)" }}>
      {toast && (
      ...
      <main className="max-w-5xl mx-auto w-full px-4 sm:px-6 py-6 space-y-3 flex-1">
        ...
        {!loading && results.length === 0 && (
          <div className="text-center py-24 text-muted-foreground text-sm">No results found. Try adjusting your filters.</div>
        )}
        {!loading && results.map((s, i) => (
          <ResultRow key={i} stock={s} onAdd={addToWatchlist} />
        ))}
      </main>
    </div>
  );
