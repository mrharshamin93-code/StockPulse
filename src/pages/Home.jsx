...
export default function Home() {
  ...
  const priceRefreshCooldown = 30_000; // 30s minimum between price refreshes

  const loadStocks = useCallback(async ({ refreshPrices = false } = {}) => {
    if (!user?.id) return;
    const data = await base44.entities.Stock.filter({ created_by_id: user.id }, "-created_date");
    setStocks(data);
    // Only refresh prices if explicitly requested or cooldown has passed
    const now = Date.now();
    if (data.length > 0 && (refreshPrices || now - lastPriceRefresh.current > priceRefreshCooldown)) {
      lastPriceRefresh.current = now;
      const tickers = data.map(s => s.ticker.toUpperCase());
      base44.functions.invoke("finnhub", { action: "quotes", tickers })
        .then(async (res) => {
          const quotes = res?.data?.quotes;
          if (!quotes?.length) return;
          const updates = quotes
            .map(q => {
              const stock = data.find(s => s.ticker.toUpperCase() === q.ticker);
              return stock && q.c ? { id: stock.id, current_price: q.c } : null;
            })
            .filter(Boolean);
          if (updates.length > 0) {
            await base44.entities.Stock.bulkUpdate(updates);
            const refreshed = await base44.entities.Stock.filter({ created_by_id: user.id }, "-created_date");
            setStocks(refreshed);
          }
        })
        .catch(() => {}); // silently fail, stale price is fine
    }
  }, []);

  useEffect(() => {
    if (user?.id) loadStocks({ refreshPrices: true }).finally(() => setLoading(false));
  }, [user?.id]);

  // Auto-refresh when stocks are created/updated/deleted from anywhere
  useEffect(() => {
