...
import { Loader2, TrendingUp, TrendingDown, RefreshCw, Newspaper, Sparkles, Plus, X } from "lucide-react";
...
// Map period to Finnhub resolution and days back
const PERIOD_CONFIG = {
  ...
  "YTD": { resolution: "W", daysBack: null }, // special
...
async function fetchChartData(ticker, period, basePrice) {
  try {
    ...
    const res = await base44.functions.invoke("finnhub", {
  ...
  // Fallback to simple simulated data if Finnhub fails
...
// Merge two data arrays by index, combining keys
...
// Deterministic simulated market metrics seeded from ticker + price
...
// Normalize data so both series show % growth from 0
...
export default function StockDetail() {
  ...
  useEffect(() => {
    const load = async () => {
      if (isTickerRoute) {
        // Watchlist-only stock: fetch profile and price via Finnhub
        try {
          const [quoteRes, profileRes] = await Promise.all([
            base44.functions.invoke("finnhub", { action: "quote", ticker: tickerFromRoute }),
            base44.functions.invoke("finnhub", { action: "profile", ticker: tickerFromRoute }),
          ]);
        ...
        const data = await base44.entities.Stock.get(id);
        setStock(data);
      }
      setLoading(false);
    };
  ...
  const fetchAnalysis = async () => {
    ...
    const result = await base44.integrations.Core.InvokeLLM({
      prompt: `Provide 3-5 recent relevant news items for ${stock.company_name} (${stock.ticker}) stock (title + brief summary, from the last few weeks/months). Be specific with facts and recent developments.`,
  ...
  const handleBuyDone = async (qty, price) => {
    ...
    await base44.entities.StockTransaction.create({ ticker: stock.ticker.toUpperCase(), company_name: stock.company_name, type: "buy", quantity: qty, price, total: qty * price });

    if (isTickerRoute) {
      await base44.entities.Stock.create({ ticker: stock.ticker.toUpperCase(), company_name: stock.company_name, quantity: qty, purchase_price: price, current_price: currentPrice, sector: stock.sector || "", logo_url: stock.logo_url || "" });
    } else {
      await base44.entities.Stock.update(id, { quantity: newQty, purchase_price: +newAvgCost.toFixed(4), current_price: currentPrice });
  ...
  const handleSellDone = async (qty) => {
    ...
    await base44.entities.StockTransaction.create({ ticker: stock.ticker.toUpperCase(), company_name: stock.company_name, type: "sell", quantity: qty, price: sellPrice, total: qty * sellPrice });
    if (qty >= stock.quantity) {
      await base44.entities.Stock.deleteMany({ ticker: stock.ticker });
    } else {
      await base44.entities.Stock.update(id, { quantity: remainingQty });
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      {/* Fixed back button — always visible while scrolling */}
      <div
        ...
        {analyzing ? (
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={fetchAnalysis} className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-gray-900 transition-colors">
            ...
            <div className="space-y-4">
              {analysis.news.map((item, i) => (
                <div key={i}>
                  ...
                  {i < analysis.news.length - 1 && <div className="border-b border-gray-100 mt-4" />}
                </div>
              ))}
            </div>
          </div>
