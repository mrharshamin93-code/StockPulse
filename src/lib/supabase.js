...
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error(
    "Missing Supabase environment variables. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY for this build.",
  );
}
...
const rawSupabase = createClient(
  ...
  supabaseAnonKey,
  {
    auth: {
      ...
      skipAutoInitialize: isOAuthCallback,
      flowType: "pkce",
    },
  },
);
...
export function persistAppleAuthorizationFromSession(session) {
  if (
    typeof window === "undefined" ||
...
function getAppleRevocationToken() {
  ...
  try {
    ...
    // Edge Function will request reauthentication if needed.
  }
...
rawSupabase.auth.onAuthStateChange((event, session) => {
  if (session) {
    persistAppleAuthorizationFromSession(session);
  }
...
function sanitizeStockSingleResult(result) {
  ...
  return {
    ...
    error: {
      ...result.error,
      message: "Unable to load this stock. Please try again.",
    },
  };
}

function wrapStocksBuilder(builder) {
  ...
  return new Proxy(builder, {
    get(target, property) {
      if (property === "single") {
        return (...args) =>
          Promise.resolve(target.single(...args)).then(
...
async function invokeDeleteAccount(options = {}) {
  try {
    const {
      data: sessionData,
      error: sessionError,
    ...
    const appleToken = getAppleRevocationToken();
    const body = {
      ...(options?.body || {}),
    };

    if (appleToken) {
      body.appleProviderToken = appleToken.token;
      body.appleProviderTokenType = appleToken.type;
    }
      ...
      if (property === "invoke") {
        return async (functionName, options = {}) => {
          if (functionName === "delete-account") {
            return invokeDeleteAccount(options);
          }
...
const wrappedFunctions = wrapFunctionsClient(
