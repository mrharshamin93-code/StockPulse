...
function BuyDialog({ open, onOpenChange, stock, onDone }) {
  ...
  const [purchasePrice, setPurchasePrice] = useState(stock?.current_price?.toFixed(2) || stock?.purchase_price?.toFixed(2) || "");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <p className="text-sm text-muted-foreground -mt-2">{stock?.company_name} · {max} shares held</p>
        <form onSubmit={handleSubmit} className="space-y-5 pt-2">
          <div className="space-y-2">
        ...
        <Link to={`/stock/${stock.id}`}>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <p className="font-semibold text-sm text-gray-900">${totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            </div>
          </div>
